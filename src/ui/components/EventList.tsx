...
export function EventList({ camId, events, filterOff, onPick }: { camId: string; events: SentinelEvent[]; filterOff: Record<string, boolean>; onPick: (ev: SentinelEvent) => void }) {
  ...
  return (
    <div className="nvr-evlist">
      {evs.map((ev) => {
        const d = new Date(ev.timestamp).toDateString(); const head = d !== lastDay; lastDay = d;
        return (
          <Fragment key={ev.id}>
