...
const ICONS: Record<SentinelEventClass, ComponentType<{ size?: number; strokeWidth?: number }>> = {
  person: User, car: Car, bike: Bike, animal: PawPrint, package: Package, motion: Activity,
};

export function classLabel(t: TFn, cls: SentinelEventClass): string { return t(`nvr.class.${cls}`); }
...
export function ClassBadge({ cls, size = 18, title }: { cls: SentinelEventClass; size?: number | undefined; title?: string | undefined }) {
  ...
  return (
    <span className={`nvr-cbadge nvr-cbadge--${cls}`} style={{ width: size, height: size }} title={title} aria-hidden={title ? undefined : true}>
...
export function EventBadges({ ev, size, t }: { ev: { classes?: string[] | undefined }; size?: number; t: TFn }) {
  return <>{sentinelClassesOf(ev).map((c) => <ClassBadge key={c} cls={c} size={size} title={classLabel(t, c)} />)}</>;
}
