...
export function VerticalTimeline(p: TimelineProps) {
  ...
  const hhmm = (ts: number) => fmtTime(ts, locale), hhmmss = (ts: number) => fmtTimeSec(ts, locale), dayLbl = (ts: number) => dayLabel(ts, locale);
      ...
      <div className="vtl-scroll" ref={scroll} onScroll={onScroll} onPointerDown={down} onTouchStart={down}>
        <div className="vtl-content" style={{ height: span * px + PAD + PAD_BOT }}>
          ...
          {vis.map(ev => {
            ...
            return (
              <div key={ev.id}>
                <button className={'vev' + (thumb ? '' : ' vev--minor')} style={{ top: y, background: `var(--nvr-c-${k})` }} onClick={() => p.onEvent(ev)} aria-label={`${classLabel(t, k)} ${hhmmss(ev.timestamp)}`} title={hhmmss(eventPlayTs(ev))} />
                {thumb && <>
                  <div className="vlink" style={{ top: y }}><span className="vlink-badges"><EventBadges ev={ev} t={t} /></span><i /></div>
                  <button className="vthumb" style={{ top: y }} onClick={() => p.onEvent(ev)}>
                    ...
                    <span className="tt nvr-data">{hhmmss(ev.timestamp)}</span></button>
                </>}
              </div>
            );
          })}
          {inWin(now) && <><div className="vlive-line" style={{ top: yFor(now) }} /><button className="vlive-chip" style={{ top: yFor(now) }} onClick={p.onGoLive}>{t('nvr.live')}</button></>}
        </div>
      </div>
      <div className="vcenter"><span className="vcenter-t nvr-data" data-ts={Math.round(center)}>{centerIsToday ? '' : dayLbl(center) + ' '}{hhmmss(center)}</span></div>
      <div className="vtl-zoom">
        <button type="button" className="tool" onClick={() => zoomStep(1.7)} aria-label={t('nvr.timeline.zoomIn')}><Plus size={16} /></button>
        <button type="button" className="tool" onClick={() => zoomStep(1 / 1.7)} aria-label={t('nvr.timeline.zoomOut')}><Minus size={16} /></button>
      </div>
      {!p.live && <button type="button" className="livejump" onClick={p.onGoLive}><ArrowUp size={14} /> {t('nvr.live')}</button>}
    </div>
  );
}
