...
export function useDatePicker({ dayStart, timeTs, oldestAllowed = -Infinity, onGo }: DatePickerPanelProps) {
  ...
  const cells = Array.from({ length: 42 }, (_, i) => {
    const dnum = i - startIdx + 1; const d = new Date(y, m, dnum);
    return { d, out: dnum < 1 || dnum > dim, off: d.getTime() > today.getTime() || d.getTime() < oldestAllowed, isToday: d.getTime() === today.getTime(),
      sel: d.getFullYear() === sel.getFullYear() && d.getMonth() === sel.getMonth() && d.getDate() === sel.getDate() };
  });
  const submit = () => { onGo(sel.getTime(), /^(\d{1,2}):(\d{2})$/.test(time) ? time : null); };
  ...
  const grid = (
    <>
      ...
      <div className="nvr-dt__grid">
        ...
        {cells.map((c, i) => (
          <button key={i} type="button" disabled={c.out || c.off} onClick={() => setSel(c.d)}
            className={'nvr-dt__d nvr-data' + (c.out ? ' nvr-dt__d--dim' : '') + (c.off ? ' nvr-dt__d--off' : '') + (c.isToday && !c.out ? ' nvr-dt__d--today' : '') + (c.sel && !c.out ? ' nvr-dt__d--sel' : '')}>
            {c.d.getDate()}
          </button>
        ))}
      </div>
  ...
  const footer = (
    <div className="nvr-dt__foot">
      <button type="button" className="nvr-btn nvr-btn--ghost" onClick={goToday}>{t('nvr.date.today')}</button>
      <input className="nvr-input nvr-dt__time nvr-data" type="time" value={time} onChange={(e) => setTime(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') submit(); }} aria-label={t('nvr.date.time')} />
      <button type="button" className="nvr-btn nvr-btn--primary" onClick={submit}>{t('nvr.date.go')}</button>
    </div>
  );
...
export function DatePickerPanel(props: DatePickerPanelProps) {
  ...
  return <div className="nvr-dt">{grid}{footer}</div>;
}
