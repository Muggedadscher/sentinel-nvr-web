...
export function stageStatus(ps: Pick<PlayerState, 'live' | 'label' | 'playhead'> & Partial<Pick<PlayerState, 'transport'>>, t: TFn, locale: string, loadError = false, now: number = Date.now()): string {
  let label = loadError ? t('nvr.error.loadFailed') : ps.label ? t(`nvr.player.${ps.label}`) : '';
  if (!ps.live && !loadError && label && (ps.transport === 'mse' || ps.transport === 'native')) label += ` (${t('nvr.player.fallbackMode')})`;
...
export function shouldHandleKey(e: Pick<KeyboardEvent, 'key' | 'ctrlKey' | 'metaKey' | 'altKey' | 'defaultPrevented' | 'target'>): boolean {
  ...
  if (e.key === ' ' && (tag === 'button' || tag === 'a' || tag === 'summary' || el.getAttribute?.('role') === 'button')) return false;
  return true;
}
