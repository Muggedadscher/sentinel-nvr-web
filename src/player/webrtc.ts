...
export function mobileClient(): boolean {
  return (
    ...
    (navigator.maxTouchPoints > 1 && Math.min(screen.width || 9999, screen.height || 9999) < 920)
  );
}
function rtcOptions() {
  let capabilities: unknown;
  try {
    capabilities = RTCRtpReceiver.getCapabilities
      ? { audio: RTCRtpReceiver.getCapabilities('audio'), video: RTCRtpReceiver.getCapabilities('video') }
      : undefined;
  } catch {
    /* none */
  }
...
export class WebRtcSession {
  ...
  id: string | undefined;
  active = false;
  private connectT: number | undefined;
  private trackSig = '';
  ...
  start(): void {
    ...
    this.connectT = window.setTimeout(() => {
      if (this.active && this.ws === ws && !this.connected) {
        ...
        if (!relayForced()) {
          forceRelayUntil = Date.now() + FORCE_RELAY_MS;
    ...
    ws.onmessage = (ev) => {
      ...
      else if (msg.type === 'addIceCandidate')
  ...
  private ensurePc(setup: any): RTCPeerConnection {
    ...
    let cfg: RTCConfiguration = (setup && setup.configuration) || {
            ...
            pc.getStats()
              .then((s) => {
                ...
                if (dec <= 0) {
                  ...
                  if (!relayForced() && vBytes <= 0 && aBytes <= 0) {
                    forceRelayUntil = Date.now() + FORCE_RELAY_MS;
                    rlog('retry-relay-only', { kind: this.opts.mode });
                    this.stop();
                    this.start();
  ...
  private createLocal(ws: WebSocket, msg: any): void {
    ...
    const reply = (desc: RTCSessionDescriptionInit) => {
      try {
        ws.send(
          JSON.stringify({ type: 'response', reqId: msg.reqId, description: { type: desc.type, sdp: desc.sdp } }),
        );
      } catch {
        /* ignore */
      }
    ...
    const p =
      msg.dtype === 'offer'
        ? pc
            .createOffer({ offerToReceiveAudio: !!msg.setup?.audio, offerToReceiveVideo: !!msg.setup?.video })
            .then((o) => pc.setLocalDescription(o).then(() => reply(o)))
