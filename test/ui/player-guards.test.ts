...
describe('stageStatus fallback transport', () => {
  ...
  it('recorded playback on MSE / native says so; relay and live do not', () => {
    ...
    expect(
      stageStatus({ live: false, label: 'playing', playhead: now, transport: 'mse' }, t, 'de-DE', false, now),
    ).toBe('[nvr.player.playing] ([nvr.player.fallbackMode]) · 12:00:00');
    expect(
      stageStatus({ live: false, label: 'playing', playhead: now, transport: 'relay' }, t, 'de-DE', false, now),
    ).toBe('[nvr.player.playing] · 12:00:00');
    expect(
      stageStatus({ live: true, label: 'liveMse', playhead: null, transport: 'mse' }, t, 'de-DE', false, now),
    ).toBe('[nvr.player.liveMse]');
  });
});
